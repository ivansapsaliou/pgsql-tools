...
exports.HealthCommands = void 0;
const vscode = __importStar(require("vscode"));
function escHtml(s) {
    return String(s ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
}
function buildTable(columns, rows, badgeCol) {
    if (rows.length === 0) {
    ...
    const bodyRows = rows.map(row => {
        const cells = columns.map(col => {
            ...
            if (col === badgeCol) {
                ...
                if (low.includes('vacuum') || low.includes('analyze'))
                    cls = 'badge-warn';
                else if (low === 'ok')
                    cls = 'badge-ok';
                else if (low.includes('critical') || low.includes('never'))
                    cls = 'badge-crit';
            }
...
function wrapBody(title, subtitle, tableHtml) {
...
async function ensureActiveConnection(connectionManager) {
...
class HealthCommands {
    static registerAll(queryExecutor, connectionManager, resultsViewProvider) {
        return [
    ...
    // ── Slow Queries ──────────────────────────────────────────────────────────
    static registerSlowQueries(queryExecutor, connectionManager, resultsViewProvider) {
        return vscode.commands.registerCommand('pgsql-tools.healthSlowQueries', async () => {
            ...
            try {
                await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Loading slow queries…', cancellable: false }, async () => {
                    // Check if pg_stat_statements is available
                    const extCheck = await queryExecutor.executeQuery(`SELECT EXISTS(SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') AS available`);
    ...
    // ── Locks ─────────────────────────────────────────────────────────────────
    static registerLocks(queryExecutor, connectionManager, resultsViewProvider) {
        return vscode.commands.registerCommand('pgsql-tools.healthLocks', async () => {
            const connName = await ensureActiveConnection(connectionManager);
            if (!connName)
                return;
            try {
                await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Loading lock information…', cancellable: false }, async () => {
                    const res = await queryExecutor.executeQuery(`
							SELECT
    ...
    // ── Sizes ─────────────────────────────────────────────────────────────────
    static registerSizes(queryExecutor, connectionManager, resultsViewProvider) {
        return vscode.commands.registerCommand('pgsql-tools.healthSizes', async () => {
            ...
            try {
                await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Loading table sizes…', cancellable: false }, async () => {
                    const res = await queryExecutor.executeQuery(`
							SELECT
    ...
    static registerVacuum(queryExecutor, connectionManager, resultsViewProvider) {
        return vscode.commands.registerCommand('pgsql-tools.healthVacuum', async () => {
            const connName = await ensureActiveConnection(connectionManager);
            if (!connName)
                return;
            try {
                await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Loading vacuum stats…', cancellable: false }, async () => {
                    const res = await queryExecutor.executeQuery(`
							SELECT
								schemaname                  AS schema,
								relname                     AS table,
...
exports.HealthCommands = HealthCommands;
//# sourceMappingURL=healthCommands.js.map