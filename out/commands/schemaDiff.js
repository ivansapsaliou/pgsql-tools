...
async function getSchemaSnapshot(client, queryExecutor, schema) {
    const eq = (q) => queryExecutor.executeQueryOnClient(client, q);
    const esc = (s) => s.replace(/'/g, "''");
    const tablesRes = await eq(`SELECT table_name FROM information_schema.tables WHERE table_schema = '${esc(schema)}' AND table_type = 'BASE TABLE' ORDER BY table_name`);
    // Fetch all columns for the schema in one query to avoid N+1 round trips
    const allColsRes = await eq(`SELECT table_name, column_name, data_type, is_nullable, column_default FROM information_schema.columns WHERE table_schema = '${esc(schema)}' ORDER BY table_name, ordinal_position`);
    const tables = {};
    ...
    const idxRes = await eq(`SELECT schemaname, tablename, indexname, indexdef FROM pg_indexes WHERE schemaname = '${esc(schema)}' ORDER BY tablename, indexname`);
    const conRes = await eq(`SELECT tc.table_name, tc.constraint_name, tc.constraint_type FROM information_schema.table_constraints tc WHERE tc.table_schema = '${esc(schema)}' ORDER BY tc.table_name, tc.constraint_name`);
    const viewRes = await eq(`SELECT table_name, view_definition FROM information_schema.views WHERE table_schema = '${esc(schema)}' ORDER BY table_name`);
    const enumRes = await eq(`SELECT t.typname, string_agg(e.enumlabel, ',' ORDER BY e.enumsortorder) AS labels FROM pg_type t JOIN pg_enum e ON e.enumtypid = t.oid JOIN pg_namespace n ON n.oid = t.typnamespace WHERE n.nspname = '${esc(schema)}' GROUP BY t.typname ORDER BY t.typname`);
    return {
        tables,
        indexes: idxRes.rows,
...
function diffSchemas(labelA, a, labelB, b) {
    const sections = [];
    // ── Tables ──
    ...
    for (const t of commonTables) {
        ...
        for (const [col, meta] of colMapA) {
            if (!colMapB.has(col)) {
                ...
                const diffs = [];
                if (meta.data_type !== metaB.data_type)
                    diffs.push(`type: ${escHtml(meta.data_type)} → ${escHtml(metaB.data_type)}`);
                if (meta.is_nullable !== metaB.is_nullable)
                    diffs.push(`nullable: ${meta.is_nullable} → ${metaB.is_nullable}`);
    ...
    const buildSection = (sectionTitle, headers, rows) => {
        if (rows.length === 0) {
        ...
        return vscode.commands.registerCommand('pgsql-tools.schemaDiff', async () => {
            ...
            try {
                if (mode.value === 'db') {
                    ...
                    const connAItem = await vscode.window.showQuickPick(connections.map(c => ({ label: c })), { title: 'Schema Diff — Select Connection A', placeHolder: 'First connection' });
                    if (!connAItem)
                        return;
                    const connBItems = connections.filter(c => c !== connAItem.label);
                    const connBItem = await vscode.window.showQuickPick(connBItems.map(c => ({ label: c })), { title: 'Schema Diff — Select Connection B', placeHolder: 'Second connection' });
                    if (!connBItem)
                        return;
                    ...
                    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Schema Diff', cancellable: false }, async (progress) => {
                        ...
                        const html = diffSchemas(`${connAItem.label} (${schemaName})`, snapA, `${connBItem.label} (${schemaName})`, snapB);
                    ...
                    // ── Schema vs Schema ──
                    const connItem = await vscode.window.showQuickPick(connections.map(c => ({ label: c })), { title: 'Schema Diff — Select Connection', placeHolder: 'Connection to use' });
                    if (!connItem)
                        return;
                    ...
                    const schemaAItem = await vscode.window.showQuickPick(schemaNames.map(s => ({ label: s })), { title: 'Schema Diff — Select Schema A', placeHolder: 'First schema' });
                    if (!schemaAItem)
                        return;
                    const schemaBItems = schemaNames.filter(s => s !== schemaAItem.label);
                    const schemaBItem = await vscode.window.showQuickPick(schemaBItems.map(s => ({ label: s })), { title: 'Schema Diff — Select Schema B', placeHolder: 'Second schema' });
                    if (!schemaBItem)
                        return;
                    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Schema Diff', cancellable: false }, async (progress) => {
...
//# sourceMappingURL=schemaDiff.js.map