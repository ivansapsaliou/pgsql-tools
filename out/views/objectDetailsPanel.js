...
class ObjectDetailsPanel {
    static async show(context, schema, objectName, objectType, queryExecutor, connectionManager, resultsViewProvider) {
        ...
        try {
            if (objectType === 'table') {
                const [ddl, data, totalCount, indexes, foreignKeys, constraints, columnDetails] = await Promise.all([
                ...
                this.currentPanel.webview.html = this.getHtml(schema, objectName, ddl, data, totalCount, indexes, foreignKeys, constraints, columnDetails);
            }
        }
        catch (err) {
    ...
    static async fetchColumnDetails(queryExecutor, schema, tableName) {
    ...
    // ── HTML ──────────────────────────────────────────────────────────────────
    static getHtml(schema, tableName, ddl, data, totalCount, indexes, foreignKeys, constraints, columnDetails) {
        ...
        const columnsTabHtml = columnDetails.map((col) => {
            ...
            if (col.is_unique && !col.is_pk)
                badges.push(`<span class="badge badge--uq">UQ</span>`);
            if (col.fk_table)
                badges.push(`<span class="badge badge--fk">FK</span>`);
            if (col.notnull && !col.is_pk)
                badges.push(`<span class="badge badge--nn">NOT NULL</span>`);
