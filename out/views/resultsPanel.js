...
class ResultsViewProvider {
    ...
    async show(results, queryExecutor, connectionManager) {
    ...
    updateUI() {
        if (!this._view || !this.currentResults)
            return;
        this._view.webview.html = this.getHtml(this.currentResults);
    }
    generateUpdateQueries(results) {
        const queries = [];
    ...
    getHtml(results) {
        ...
        const headerHtml = columns.map(col => `<th class="sortable" data-column="${col}"><span class="th-label">${this.escapeHtml(col)}</span><span class="sort-icon"></span></th>`).join('');
        return `<!DOCTYPE html>
	<html>
	<head>
	<meta charset="UTF-8">
...
ResultsViewProvider.viewType = 'pgsqlResults';
