...
class QueryExecutor {
    ...
    async getTableDDL(schema, tableName) {
        ...
        const conRes = await this.executeQuery(`
			...
			ORDER  BY
				...
				con.conname
		`);
        const constraintLines = conRes.rows.map(row => `    CONSTRAINT "${row.name}" ${row.def}`);
        ...
        if (colComments.length > 0) {
            ddl += '\n';
            ddl += colComments.map(r => `COMMENT ON COLUMN "${schema}"."${tableName}"."${r.col}" IS '${String(r.col_comment).replace(/'/g, "''")}';`).join('\n');
        }
        return ddl;
    }
    async getTableData(schema, tableName, limit = 100, offset = 0) {
    ...
    async getIndexes(schema, tableName) {
    ...
    async getForeignKeys(schema, tableName) {
    ...
    async getConstraints(schema, tableName) {
        const result = await this.executeQuery(`
			SELECT
				tc.constraint_name,
				tc.constraint_type,
