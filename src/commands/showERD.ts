...
function generateErdHtml(
	...
	const tableBoxes = tables.map(tableName => {
		...
		const refRows = refs.map(fk =>
			`<div class="erd-ref-row">→ ${escHtml(fk.source_column)} ▶ ${escHtml(fk.target_table)}.${escHtml(fk.target_column)}</div>`
		).join('');
		...
		return vscode.commands.registerCommand('pgsql-tools.showERD', async () => {
			...
			if (connections.length === 1) {
				...
				const connItem = await vscode.window.showQuickPick(
					...
					{ title: 'ER Diagram — Select Connection', placeHolder: 'Choose connection' }
				);
				if (!connItem) return;
			...
			if (schemaNames.length > 1) {
				const schemaItem = await vscode.window.showQuickPick(
					schemaNames.map(s => ({ label: s })),
					{ title: 'ER Diagram — Select Schema', placeHolder: 'Choose schema (default: public)' }
				);
				...
				await vscode.window.withProgress(
					...
					async (progress) => {
						...
						const tablesRes = await queryExecutor.executeQueryOnClient(
							client,
							`SELECT table_name FROM information_schema.tables WHERE table_schema = '${esc(schemaName)}' AND table_type = 'BASE TABLE' ORDER BY table_name`
						);
						...
						const colsRes = await queryExecutor.executeQueryOnClient(
							client,
							`SELECT table_name, column_name, data_type, is_nullable FROM information_schema.columns WHERE table_schema = '${esc(schemaName)}' ORDER BY table_name, ordinal_position`
						);

						progress.report({ message: 'Loading primary keys…' });
						const pksRes = await queryExecutor.executeQueryOnClient(
							client,
							`SELECT tc.table_name, kcu.column_name FROM information_schema.table_constraints tc JOIN information_schema.key_column_usage kcu ON kcu.constraint_name = tc.constraint_name AND kcu.table_schema = tc.table_schema WHERE tc.constraint_type = 'PRIMARY KEY' AND tc.table_schema = '${esc(schemaName)}'`
						);
