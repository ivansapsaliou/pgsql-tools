...
function renderPlanNode(node: any, depth: number = 0): string {
	...
	const children = (node['Plans'] ?? []).map((child: any) => renderPlanNode(child, depth + 1)).join('');
	return header + children;
}

export class ExplainQueryCommand {
	static register(
		...
		return vscode.commands.registerCommand('pgsql-tools.explainQuery', async () => {
			...
			if (analyze) {
				...
				if (!buffersItem) return;
				buffers = buffersItem.value;
			}

			try {
				await vscode.window.withProgress(
					{ location: vscode.ProgressLocation.Notification, title: 'Running EXPLAIN…', cancellable: false },
					async () => {
