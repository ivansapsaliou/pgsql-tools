...
export function registerHealthSlowQueries(parent: Command): void {
  parent
    ...
    .action(async (opts: {
      ...
      try {
        ...
        const extCheck = await query<{ count: string }>(client, `
        ...
        const rows = await query<SlowQueryRow>(client, `
          SELECT
            calls::text,
...
export function registerHealthLocks(parent: Command): void {
  parent
    .command('locks')
    ...
    .action(async (opts: {
      ...
      try {
        const rows = await query<LockRow>(client, `
          SELECT
...
export function registerHealthSizes(parent: Command): void {
  parent
    .command('sizes')
    ...
    .action(async (opts: {
      ...
      try {
        ...
        const rows = await query<SizeRow>(client, `
          SELECT
...
export function registerHealthVacuum(parent: Command): void {
  parent
    .command('vacuum')
    ...
    .action(async (opts: {
      ...
      try {
        ...
        const rows = await query<VacuumRow>(client, `
          SELECT
