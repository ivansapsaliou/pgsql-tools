...
async function introspectSchema(client: Client, schemaName: string): Promise<SchemaSnapshot> {
  const tables = await query<TableInfo>(client, `
  ...
  for (const t of tables) {
    ...
    columns[key] = await query<ColumnInfo>(client, `
  ...
  const indexes = await query<IndexInfo>(client, `
  ...
  const constraints = await query<ConstraintInfo>(client, `
  ...
  const enums = await query<EnumInfo>(client, `
  ...
  const views = await query<ViewInfo>(client, `
...
interface DiffResult {
  tables: { added: string[]; removed: string[]; changed: ChangedTable[] };
...
function diffSchemas(
  ...
  const changedEnums = [...enumNamesA.keys()].filter(
    k => enumNamesB.has(k) && enumNamesA.get(k) !== enumNamesB.get(k)
  );
  ...
  const changedViews = [...viewNamesA.keys()].filter(
    k => viewNamesB.has(k) && normalizeWs(viewNamesA.get(k)!) !== normalizeWs(viewNamesB.get(k)!)
  );
  ...
  const changedIndexes = [...idxKeyA.keys()].filter(
    k => idxKeyB.has(k) && idxKeyA.get(k) !== idxKeyB.get(k)
  );
  ...
  const changedConstraints = [...conKeyA.keys()].filter(
    k => conKeyB.has(k) && conKeyA.get(k) !== conKeyB.get(k)
  );
...
function printDiff(diff: DiffResult, labelA: string, labelB: string): void {
  ...
  function added(msg: string): void {
    hasChanges = true;
    console.log(`  + ${msg}`);
  }
  function removed(msg: string): void {
    hasChanges = true;
    console.log(`  - ${msg}`);
  }
  ...
  if (diff.tables.added.length || diff.tables.removed.length || diff.tables.changed.length) {
    section('TABLES');
    for (const t of diff.tables.added) added(`table ${t} (only in ${labelB})`);
    for (const t of diff.tables.removed) removed(`table ${t} (only in ${labelA})`);
    for (const t of diff.tables.changed) {
      changed(`table ${t.name}`);
      for (const c of t.addedColumns) console.log(`      + column ${c} (only in ${labelB})`);
      for (const c of t.removedColumns) console.log(`      - column ${c} (only in ${labelA})`);
      for (const c of t.changedColumns) console.log(`      ~ column ${c} (type/nullable/default changed)`);
    }
  }
  ...
  if (diff.enums.added.length || diff.enums.removed.length || diff.enums.changed.length) {
    section('ENUMS');
    for (const e of diff.enums.added) added(`enum ${e} (only in ${labelB})`);
    for (const e of diff.enums.removed) removed(`enum ${e} (only in ${labelA})`);
    for (const e of diff.enums.changed) changed(`enum ${e} (values changed)`);
  }

  // Views
  if (diff.views.added.length || diff.views.removed.length || diff.views.changed.length) {
    section('VIEWS');
    for (const v of diff.views.added) added(`view ${v} (only in ${labelB})`);
    for (const v of diff.views.removed) removed(`view ${v} (only in ${labelA})`);
    for (const v of diff.views.changed) changed(`view ${v} (definition changed)`);
  }

  // Indexes
  if (diff.indexes.added.length || diff.indexes.removed.length || diff.indexes.changed.length) {
    section('INDEXES');
    for (const i of diff.indexes.added) added(`index ${i} (only in ${labelB})`);
    for (const i of diff.indexes.removed) removed(`index ${i} (only in ${labelA})`);
    for (const i of diff.indexes.changed) changed(`index ${i} (definition changed)`);
  }

  // Constraints
  if (diff.constraints.added.length || diff.constraints.removed.length || diff.constraints.changed.length) {
    section('CONSTRAINTS');
    for (const c of diff.constraints.added) added(`constraint ${c} (only in ${labelB})`);
    for (const c of diff.constraints.removed) removed(`constraint ${c} (only in ${labelA})`);
    for (const c of diff.constraints.changed) changed(`constraint ${c} (definition changed)`);
  }
...
export function registerSchemaDiff(parent: Command): void {
  parent
    .command('diff')
    ...
    .action(async (opts: {
      ...
      let clientA: import('pg').Client | null = null;
      let clientB: import('pg').Client | null = null;

      try {
        clientA = await connect(urlA);
        clientB = urlB === urlA ? clientA : await connect(urlB);
        ...
        const rekeyedA: SchemaSnapshot = {
          ...snapshotA,
          columns: Object.fromEntries(
            Object.entries(snapshotA.columns).map(([k, v]) => [k.replace(/^[^.]+\./, `${schemaA}.`), v])
          ),
        };
        const rekeyedB: SchemaSnapshot = {
          ...snapshotB,
          columns: Object.fromEntries(
            Object.entries(snapshotB.columns).map(([k, v]) => [k.replace(/^[^.]+\./, `${schemaB}.`), v])
          ),
        };
        ...
        if (clientA) await disconnect(clientA);
        if (clientB && clientB !== clientA) await disconnect(clientB);
      }
    });
}
